...
public class PerlinNoise2D {
	...
	public PerlinNoise2D(GamePanel gp) {
		
		this.gp = gp;
		image = new BufferedImage(gp.worldWidth, gp.worldHeight, BufferedImage.TYPE_INT_RGB);
	}
    
	public BufferedImage getNoiseImage(){
    	time += 0.01;
    	for(int y = 0; y < gp.worldHeight; y++){
    		for(int x = 0; x < gp.worldWidth; x++){
    			double dx = (double) x / gp.worldHeight;
    			double dy = (double) y / gp.worldHeight;
    			int frequency = 6;
